...
std::vector<PosInfo> cog_obstacles_to_PosInfo(Car::CarStateValues &sensing_info, std::vector<PosInfo> waypoints) {
	...
	auto &obs = sensing_info.track_forward_obstacles;

	std::transform(obs.begin(), obs.end(), std::back_inserter(ret), [&sensing_info, &waypoints](Car::ObstaclesInfo obs) -> PosInfo {
